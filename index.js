 ...
 */
function processLastItem(sl, cb) {
  return cb(sl[sl.length - 1]);
}
 ...
 * should return 1000.
 */
function processSum(nl, cb) {
  var res = nl.reduce((acc, nl) => acc + nl, 0);
  return cb(res);
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(el) {
    solu2.push(`${el.last_name}, ${el.first_name}`);
  });
  return solu2;
}
...
function getRunnersByTShirtSize(runners, size) {
  var newArray = runners.filter(el => el.shirt_size === size);
  return newArray;
}
...
function tallyUpDonations(runners) {
  const total = runners.reduce(function(acc, curr) {
